...
A Stream ends when an End Frame is successfully authenticated. An End
Frame may be full-length or partial; EOF determines the length of a
partial End Frame. Bytes following a full-length End Frame are not
part of the Stream.
...
An End Frame's serialized length MUST be within:

```
Frame Overhead <= End Frame Length <= Frame Length
```

A full-length End Frame has a serialized length of Frame Length. EOF
determines the serialized length of a partial End Frame.
...
2. Process each Frame:
    ...
    2. Determine the Frame length.
        ...
        1. If the End Frame flag is clear, the Frame is a Body
           Frame. Its total serialized length MUST equal Frame Length,
           and its payload length MUST equal:
           ```
           Frame Length - Frame Overhead
           ```
        2. If the End Frame flag is set, the Frame is an End
           Frame. Read until either Frame Length bytes have been
           received or the input reaches EOF. Before separating the
           Payload and Authentication Tag, require its serialized
           length to be within:
           ```
           Frame Overhead <= End Frame Length <= Frame Length
           ```
           The final Tag Length bytes are the Authentication Tag, and
           all preceding bytes after the Sequence Number are the
           Payload. The payload length MUST be within:
           ```
           0 <= Payload Length <= Frame Length - Frame Overhead
           ```
           If the End Frame reaches Frame Length and authenticates, stop
           without reading subsequent bytes from the enclosing input.
...
3. Validate Stream termination.
    ...
    * Bytes in an enclosing input after a full-length End Frame are not
      part of the Stream and are not authenticated by the Stream.
